...
### 길이 선택 판단 기준
- 길이는 대상 연령대가 아니라 **원전의 필수 사건량**으로 정한다. 6–7세 대상이어도 원전 핵심 장면이 많으면 `long` 또는 `short_series` 이상을 선택한다.
- `short`는 하나의 사건/반전/감정 변화만 있는 짧은 우화에 사용한다.
- `medium`은 도입-사건-해결이 단순하고 필수 장면이 2개 안팎인 이야기만 사용한다. 유명 원전 전체를 담는 기본값으로 쓰지 않는다.
- `long`은 필수 장면이 3–5개이지만 한 번에 읽는 단편으로 유지할 수 있는 이야기용이다.
- `short_series`는 여정, 반복 시련, 조력자/적대자 변화, 관계 변화가 단계적으로 있어 3–4화 분절이 자연스러운 이야기용이다.
- `long_series`는 장편 원전, 여러 장소/시련/인물군을 거치는 모험담, 성장 서사, 역사/종교/신화 서사처럼 5화 이상이 자연스러운 이야기용이다.
- 회심/개과천선, 재판, 추적/탈출, 피난, 소명, 긴 여정처럼 **과정 자체가 핵심인 소재**는 `medium` 이하로 낮추지 않는다.
- 단편 길이를 선택하려면 "원전 전체"가 아니라 "특정 에피소드만 발췌"한다는 coverage_scope를 명확히 쓴다.
- 안전 완화나 연령 조정 때문에 플롯의 필수 장면이 삭제될 것 같으면 길이를 올리고, 문장 난이도만 낮춘다.
